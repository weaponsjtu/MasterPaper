...
\begin{algorithmic}[1]
    ...
    \FOR{ each candidate $\mathbf{p}_i$ of node $i$ }
        \STATE set $m(\mathbf{p}_i) = \langle \beta_p^i, \phi_p(\mathbf{x}, p_i) \rangle + \langle \beta_{pa}^r, \Psi_{pa}(\mathbf{x}, P_r, a_r) \rangle$
    \ENDFOR
    \FOR{ each candidate $\mathbf{p}_j$ of parent node $j$ and $\mathbf{p}_i$ of child node $i$ }
    ...
    \FOR{ each parent-child pair ($\mathbf{p}_j^*, \mathbf{p}_i$) }
